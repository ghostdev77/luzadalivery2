...
function CategoryCard({ category, className, labelClassName }: CategoryCardProps) {
    return (
        <Link href={category.link} className="block transition-transform duration-300 ease-in-out hover:-translate-y-1">
            <Card className={cn("flex h-full flex-col items-center justify-center p-3 text-center transition-shadow duration-300 hover:shadow-xl bg-card rounded-2xl", className)}>
                <CardContent className="flex flex-col items-center justify-center p-0">
                    ...
                    <p className={cn("mt-2 w-full truncate font-semibold", labelClassName)}>
                        {category.nome}
                    </p>
                </CardContent>
            </Card>
...
export function CategoryCardRow({ categories, title, href }: CategoryCardRowProps) {
  return (
    <HorizontalRow title={title} href={href} ariaLabel={title} className="shadow-soft rounded-3xl mb-6">
      {categories.map((c) => (
